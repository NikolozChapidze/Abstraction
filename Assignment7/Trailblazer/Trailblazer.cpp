...
Set<Edge> createMaze(int numRows, int numCols) {
	Set<Edge> result;
	HashMap<int, Set<Loc>> clusters;
	HashMap<Loc, int > clusterId;
	...
	for(int i=0; i< numRows; i++){
		for(int j=0; j<numCols; j++){
			...
			clusters.put(clusters.size(), newSet);
			clusterId.put(curr, clusters.size()-1);
	...
	while(clusters.size() >= 2){
		...
		if(clusterId[next.start] != clusterId[next.end]){
			result.add(next);
			clusters[clusterId[next.start]] = clusters[clusterId[next.start]] + clusters[clusterId[next.end]];
			
			int endId = clusterId[next.end];
			foreach(Loc temp in clusters[endId]){
				clusterId[temp] = clusterId[next.start];
			}
			clusters.remove(endId);
		}
	}
    return result;
}
